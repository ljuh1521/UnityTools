...
위 부품들을 조립해 둔 프리팹 28개. `Unit/` 은 가장 작은 조각(이미지·글자), `Generic/` 은 그것을
...
DefenceR 에서 옮겨 올 때 **10개는 남겨야 했다.** 다시 조사하지 않도록 이유를 적어 둔다 —
전부 "패키지가 프로젝트 것을 물게 된다"는 같은 이유다.
...
**`Gage` · `Image_GridText` · `Toast` 는 풀려서 들어왔다**(2026-09-21). 앞의 둘은 흰 판
하나(`Capsule`)만, `Toast` 는 `CapsuleSoft` 하나만 물고 있었다 — 그 판들이 공용으로 올라오면서
막던 이유가 사라졌다. **먼저 옮긴 것이 뒤에 오는 것의 길을 연다.**

**유니티 내장 스프라이트로 갈아 끼우는 길은 답이 아니었다** — 실제로 바꿔 렌더해 보니
`HeroLevelRow` 의 묘비와 촛불이 흰 네모가 되고 자물쇠는 사라졌다. **그림 자체를 공용으로
올릴 수 있는가**로 판단한다.
