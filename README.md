...
| `Logs/Agent/console.txt` | 배치가 도는 동안의 콘솔을 모아 두는 곳. 응답에 옮겨 담고 비운다 |

**콘솔은 배치가 끝날 때까지 받는다** — `play` 로 들어가 게임이 도는 동안 난 로그도 응답의
`# 콘솔` 절에 들어온다. 로그가 256KB를 넘으면 거기서 멈추고 **끊겼다는 것을 응답에 적는다.**

**이 패키지를 고쳤으면 `recompile` 을 먼저 넣는다.** 안 넣으면 옛 코드가 도는데 겉보기에는
멀쩡하다. 잊었을 때를 대비해 배치 첫머리에서 알려 준다:
...
```
   컴파일 걸렸습니다 — 어셈블리 3개를 다시 만들었습니다.
   컴파일 걸렸습니다.                     ← 캐시로 채워져 개수를 못 셌을 때
   컴파일이 안 걸렸습니다 — 바뀐 스크립트를 찾지 못했습니다. ...
```

유니티는 밖에서 고친 `.cs`를 놓칠 때가 있다. 그러면 옛 어셈블리가 그대로 남아서, 방금 추가한
메서드를 `call`해도 "찾지 못했습니다"가 나온다 — **오타일 때와 글자 하나 다르지 않다.**
이 보고가 그 둘을 갈라 준다. 안 걸렸다고 나오는데 고친 게 분명하면 `recompile`을 넣는다 —
빌드 캐시를 지우고 전체를 다시 만들므로 반드시 걸린다. 대신 수 분 걸리니 탈출구로만 쓴다.
...
### 패키지 자립 검사 — `PackageSelfCheck`

**이 패키지가 붙인 프로젝트의 자산에 기대고 있지 않은지** 스스로 본다. 검사 모음에 끼어 있어
`Tools/유니티 툴즈/검사`를 돌릴 때마다 함께 돈다.

보는 것은 **문자열로 자산을 찾는 자리** — `Resources.Load("…")` · `Shader.Find("…")`. 그 이름이
패키지 안의 `Resources/`나 `.shader`에 실재하는지 대조하고, 없으면 알린다.

왜 이것만 따로 보는가 — **이 종류는 어떤 검사에도 안 걸린다.** 경로가 문자열이라 컴파일러가
안 보고, 타입 참조를 보는 검사도 프리팹 참조를 보는 검사도 문자열은 안 본다. 그런데 틀리면
`Resources.Load`가 null을 주고 **쓰는 쪽은 대개 오류 없이 아무것도 안 그린다.**

실제로 0.4.0부터 0.8.0까지, `OutlineUI`·`ShadowUI`가 프로젝트 머티리얼 둘을 붙든 채로 이관 검증을
전부 통과하고 있었다. 우연히 발견했지 잡아낸 게 아니다 — **사람이 기억해야만 발동하는 규칙은
안 지켜지므로** 검사로 만들어 뒀다.
